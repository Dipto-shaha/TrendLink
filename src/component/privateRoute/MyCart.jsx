...
const MyCart = () => {
    ...
    useEffect(() => {
        fetch(`http://localhost:5000/saveCartProduct/${user.email}`)
          .then(res => res.json())
          .then(cartItems => {
            ...
            <div className=" grid grid-cols-1 lg:grid-cols-2 lg:gap-10 gap-5">
                {info.map((item)=>
                        <div key={item._id} className="flex border-2 rounded-md">
                            <div><img className=" w-60 h-60 rounded-lg" src={item.image}></img></div>
                            ...
                            <p>Type:{item.type}</p>
                            <button className="px-5 py-2 bg-[#ff4d4d] rounded-lg mt-2 flex items-center text-[#FFF]" onClick={()=>handleDelete(item._id)} > <AiFillDelete className="mr-2"></AiFillDelete> Delete</button>
                            </div>
                        </div>
                )}
            </div>
